...
select calc.warehouse
from warehouses capa,
(select warehouse, count(*) cnt
from boxes
group by warehouse) as calc 
where capa.code = calc.warehouse
and capa.capacity < calc.cnt;
...
delete from boxes 
where warehouse in ( select calc.warehouse
								from warehouses capa,
								(select warehouse, count(*) cnt
								 from boxes
								 group by warehouse) as calc 
